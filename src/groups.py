...
class AllSprites(pygame.sprite.Group):
    def __init__(self, width, height, clouds, horizon_line, bg_tile = None, top_limit = 0):
        ...
        self.borders = {
            'left': 0,
            'right': -self.width + WINDOWN_WIDTH,
            'bottom': -self.height + WINDOWN_HEIGHT,
            'top': top_limit}
    ...
    def draw_sky(self):
        ...
        sea_rect = pygame.FRect(0,horizon_pos,WINDOWN_WIDTH,WINDOWN_HEIGHT - horizon_pos)
        pygame.draw.rect(self.display_surface, '#92a9ce', sea_rect)

        # horizon line
        pygame.draw.line(self.display_surface, '#f5f1de', (0,horizon_pos), (WINDOWN_WIDTH, horizon_pos), 4)
    ...
    def draw(self, target_pos, dt):
        self.offset.x = -(target_pos[0] - WINDOWN_WIDTH / 2)
        self.offset.y = -(target_pos[1] - WINDOWN_HEIGHT / 2)
